...
class BaseHttpMethodDecorator extends base_decorator_1.BaseDecorator {
    ...
    static handleSuccess(result, statusCode) {
        const status = statusCode || (result !== null && result !== undefined && result !== '' ? status_code_1.HttpStatusCode.OK : status_code_1.HttpStatusCode.NO_CONTENT);
    ...
    static validateMethod(expected, found) {
        if (expected !== found) {
            throw new service_1.HttpGenericError(status_code_1.HttpStatusCode.METHOD_NOT_ALLOWED, `${found} - Method not allowed!`);
        }
    }
}
