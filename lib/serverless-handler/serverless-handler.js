...
class AServerlessHandler {
    ...
    handler(controllerType, method, options) {
        return (p0, p1) => __awaiter(this, void 0, void 0, function* () {
            var _a;
            const start = new Date();
            try {
                const handler = this.getHandlerProvicer(options);
                const response = yield handler.applyCall(controllerType, method, p0, p1);
                return response;
            }
            catch (err) {
                return {
    ...
    getHandlerProvicer(options) {
        var _a;
        const provider = (_a = options.serverlessProvider) !== null && _a !== void 0 ? _a : AServerlessHandler.provider;
