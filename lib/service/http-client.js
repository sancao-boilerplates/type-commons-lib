...
class HttpClient {
    executeRequest(method, url, payload, requestConfig, requestOptions) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                log_1.Logger.debug('Request executed ', { duration, url: config.url });
                ...
                log_1.Logger.error(`Error executing http request method:${method} to: ${config.baseURL}/${config.url}`, err.message);
    ...
    generateRequestConfig(method, url, payload, requestConfig) {
        ...
        const correlationId = log_1.LoggerContext.getCorrelationId() || (0, uuid_1.v4)();
        ...
        const config = Object.assign(Object.assign({}, requestConfig), { method,
            url, data: (payload !== null && payload !== void 0 ? payload : method == 'GET') ? undefined : payload });
        return config;
    }
    ...
    handlePayload(payload) {
        try {
            ...
            log_1.Logger.warn(`Error while handling service payload: ${payload}`, err);
            return null;
        }
    }
    handlerError(err, config, statusCode) {
        ...
        const data = {
            url: `${config.url}`,
            baseUrl: `${config.baseURL}`,
            errorCode: `INT_ERR_CODE_${log_1.LoggerContext.getCorrelationId()}`,
            statusCode,
