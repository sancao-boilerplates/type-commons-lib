...
var RequestToCurl = /** @class */ (function () {
    ...
    RequestToCurl.convertToCurl = function (config) {
        ...
        var curl = "curl ".concat(method, " ").concat(url, " ").concat(headers, " ").concat(method !== 'GET' ? body : '').trim().replace(/\s{2,}/g, ' ');
        return curl;
    };
    ...
    RequestToCurl.getMethod = function (method) {
        return "-X ".concat(method || ''.toUpperCase());
    };
    ...
    RequestToCurl.getUrl = function (config) {
        if (config.baseURL) {
            return "".concat(config.baseURL, "/").concat(config.url);
        }
        return config.url;
    };
